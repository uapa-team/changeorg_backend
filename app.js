  ...
  if (token) {
    try {
      ...
      if (decoded.exp <= parseInt(moment().format("X"))) {
        ...
        db.all(
          "SELECT * FROM sponsor INNER JOIN request on " +
...
app.post("/recommend", (req, res) => {
  ...
  try {
    db.run(
      query,
      [req.body.name, req.body.email, req.body.type, req.body.msg],
      (err) => {
        console.log(err);
      }
    );
  ...
  if (token) {
    try {
      ...
      if (decoded.exp <= parseInt(moment().format("X"))) {
        ...
        db.run(
          "UPDATE request SET Apadrinado = 1 WHERE request_id like ?",
          [req.body.request_id],
          (error) => {
            if (error) {
                ...
                (error) => {
                  ...
                  try {
                    const emailCompiled = hbs.compile(
                      fs.readFileSync("views/email/toProfessor.hbs").toString()
                    )({
                      nombre: row.Nombre,
  ...
  if (token) {
    try {
      ...
      if (decoded.exp <= parseInt(moment().format("X"))) {
        ...
        const query =
          "SELECT request_id, Programa, Fecha, PBM, Procedencia, " +
        ...
        db.all(query, (error, r) => {
          ...
          try {
            ...
            res.status(500).send({
              error: "Error retriving values " + err.toString(),
            });
          }
        });
      }
      ...
      authenticate(req.body.username, req.body.password)
        ...
        .catch((err) => {
          if (
            ...
            res.status(203).send({ error: "Wrong user or password" });
          } else {
            res.status(500).send({ error: "Unexpected Error" });
            auth = new LdapAuth(settings.ldap);
          }
...
app.listen(port, function () {
  ...
  if (
    ...
    console.warn(
      "WARN: This service may become unresponsive " +
